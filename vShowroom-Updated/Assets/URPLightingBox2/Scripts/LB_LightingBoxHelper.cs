...
public enum LightingMode
{
	...
	BakedEnlighten,RealtimeGlobalIllumination,
	FullyRealtime
}
...
/*
	...
	void UpdateSettings(LB_LightingProfile profile)
	{
		...
		// Update Ambient
		helper.Update_Ambient (profile.Ambient_Enabled,profile.ambientLight, profile.ambientColor,profile.skyColor, profile.skyIntensity, profile.equatorColor,profile.groundColor);
...
public void Update_Vignette(bool enabled, float intensity)
	...
	public void Update_LightingMode(bool enabled, LightingMode lightingMode, float indirectBounce, int directSamples, int indirectSamples, bool aoEnabled, float aoDistance, float aoIntensityDirect, float aoIntensityIndirect, float backfaceTolerance, float resolution,Lightmap_Quality lightmapQuality, float indirectResolution)
	{
		if (enabled)
		{
			...
			Lightmapping.lightingSettings.ao = aoEnabled;
			Lightmapping.lightingSettings.aoExponentDirect = aoIntensityDirect;
			Lightmapping.lightingSettings.aoMaxDistance = aoDistance;
			Lightmapping.lightingSettings.aoExponentIndirect = aoIntensityIndirect;
			Lightmapping.lightingSettings.indirectScale = indirectBounce;
			...
			if (lightingMode == LightingMode.RealtimeGlobalIllumination)
			{
				...
				Lightmapping.bakedGI = false;
				Lightmapping.lightingSettings.lightmapper = LightingSettings.Lightmapper.ProgressiveCPU;
				Lightmapping.lightingSettings.indirectResolution = indirectResolution;
			}
#endif
		}
	}

	public void Update_Ambient(bool enabled,AmbientLight ambientLight,Color ambientColor,Color skyColor, float skyIntensity,Color equatorColor
		,Color groundColor)
	{
		if (enabled) {
