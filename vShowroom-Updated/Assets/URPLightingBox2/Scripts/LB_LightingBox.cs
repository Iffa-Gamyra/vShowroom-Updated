...
public class LB_LightingBox : EditorWindow
{
    ...
    */
	...
	public Lightmap_Quality lightmapQuality;
   ...
   // Depth of Field
	...
	void OnGUI()
	{
		...
		GUILayout.Label ("URP Lighting Box 2 - v1.2.3 Unity 2020.3.24 Dec 2021", EditorStyles.helpBox);
		...
		if (winMode == WindowMode.Part1) {
            ...
            if (ambientState) {
				...
				{
					skyIntensity = EditorGUILayout.Slider("Intenity", skyIntensity, 0, 5f);
				}

				if (ambientLightRef != ambientLight || ambientColorRef != ambientColor
					|| skyBoxRef != skyBox || skyColorRef != skyColor || skyIntensityRef != skyIntensity
					|| equatorColorRef != equatorColor || groundColorRef != groundColor
					|| Ambient_EnabledRef != Ambient_Enabled  )
				{
					helper.Update_Ambient (Ambient_Enabled,ambientLight, ambientColor,skyColor, skyIntensity, equatorColor,groundColor);
			...
			if (lightSettingsState)
			{
				...
				var lightmapQualityRef = lightmapQuality;
				var indirectResolutionRef = indirectResolution;
				...
				// Choose lightmapping quality (lightmap parameters)
				if (lightingMode != LightingMode.FullyRealtime)
				{
					EditorGUILayout.Space();
					lightmapQuality = (Lightmap_Quality)EditorGUILayout.EnumPopup("Lightmap Quality", lightmapQuality, GUILayout.Width(343));
				}
				if (lightingMode == LightingMode.RealtimeGlobalIllumination)
				{
					EditorGUILayout.Space();
					indirectResolution = EditorGUILayout.FloatField("Indirect Resolution", indirectResolution);
					EditorGUILayout.Space();
					indirectBounce = EditorGUILayout.Slider("Indirect Intensity", indirectBounce, 0, 5);
				}
					...
					|| lightingMode == LightingMode.BakedProgressiveGPU)
				{
					...
					if (lightingModeRef != lightingMode || bakedResolutionRef != bakedResolution || directSamplesRef != directSamples
						...
						|| aoDistanceRef != aoDistance || indirectBounceRef != indirectBounce 
						|| backfaceToleranceRef != backfaceTolerance || lightmapQualityRef != lightmapQuality
						|| indirectResolutionRef != indirectResolution)
					{
						helper.Update_LightingMode(Scene_Enabled, lightingMode, indirectBounce, directSamples, indirectSamples, aoEnabled, aoDistance, aoIntensityDirect, aoIntensityIndirect, backfaceTolerance, bakedResolution, lightmapQuality, indirectResolution);

						if (LB_LightingProfile)
						{
							LB_LightingProfile.lightingMode = lightingMode; 
							LB_LightingProfile.lightmapQuality = lightmapQuality;
							LB_LightingProfile.indirectResolution = indirectResolution;
				...
				if (lightingMode == LightingMode.BakedEnlighten)
				{
					...
					if (lightingModeRef != lightingMode || bakedResolutionRef != bakedResolution || aoIntensityDirectRef != aoIntensityDirect
						|| aoIntensityIndirectRef != aoIntensityIndirect || aoEnabledRef != aoEnabled
						|| aoDistanceRef != aoDistance || indirectBounceRef != indirectBounce || backfaceToleranceRef != backfaceTolerance || lightmapQualityRef != lightmapQuality || indirectResolutionRef != indirectResolution)
					{
						helper.Update_LightingMode(Scene_Enabled, lightingMode, indirectBounce, directSamples, indirectSamples, aoEnabled, aoDistance, aoIntensityDirect, aoIntensityIndirect, backfaceTolerance, bakedResolution, lightmapQuality, indirectResolution);
				...
				if (lightingModeRef != lightingMode || Scene_EnabledRef != Scene_Enabled 
					|| lightmapQualityRef != lightmapQuality 
					|| indirectResolutionRef != indirectResolution
					|| indirectBounceRef != indirectBounce)
				{
					//----------------------------------------------------------------------
    ...
    void OnLoad()
		...
		if (LB_LightingProfile) {
			...
			lightmapQuality = LB_LightingProfile.lightmapQuality;
			indirectResolution = LB_LightingProfile.indirectResolution;
